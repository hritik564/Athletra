...
- **Active Coaching Layer**: Real-time pre-recording alignment coach that activates when the user enters camera mode. Architecture: `lib/coachingScript.ts` (7 states: NOT_VISIBLE/TOO_CLOSE/TOO_FAR/NOT_SIDEWAYS/MISALIGNED_HEAD/UNSTABLE/ALIGNED with priority ordering, messages, icons, colors); `lib/coachingFilters.ts` (Adaptive 1 Euro Filter with dynamic beta = base_beta + k_vel × smoothed_velocity; EMA velocity smoothing with alpha=0.35; missing-landmark 10%-per-frame confidence decay; adaptive stability window 300-600ms based on smoothed_velocity; LandmarkFilterBank for all tracked joints); `lib/coachingLogic.ts` (shoulder-width normalization anchor; bbox_confidence < 0.5 → NOT_VISIBLE "I can't see you clearly"; no-pose → "Step into frame"; body-height-fraction for TOO_CLOSE/FAR; shoulder/hip x-separation for NOT_SIDEWAYS; head-offset/body-height ratio for MISALIGNED_HEAD; EMA velocity threshold for UNSTABLE; returns CoachingOutput with optional messageOverride); `hooks/useCoachingState.ts` (850ms polling loop; LandmarkFilterBank + decay applied per frame; raw velocity → EMA → adaptiveWindowMs(); candidate state machine with velocity-scaled stability window; 3-stage ALIGNED progression: stage 0 <800ms "Hold still…", stage 1 ~800ms "Almost there…", stage 2 ~1200ms "Perfect. Let's go." → trigger); `components/CoachingOverlay.tsx` (300ms cross-fades on all transitions; AlignedPulse breathing ring that accelerates through stages; AlignedProgressRing fills across stages; distance indicator with zone bar and position marker; sideways hint arrow for NOT_SIDEWAYS). Backend: `/api/coach/live-pose` endpoint in `routes.ts` + `quick: true` mode in `pose_detection.py` (single-image, landmarks only, no annotation, ~3× faster than full detection). Integration in `app/analyze.tsx`: `getCameraFrame()` callback (native: `takePictureAsync({quality:0.25})`, web: canvas capture from videoRef at 35% scale), overlay shown only during alignment phase (!isCapturing), auto-starts live capture on ALIGNED stage-2 with haptic success feedback
